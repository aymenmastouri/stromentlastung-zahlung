...
public final class SaeumnisRechner {
    ...
    public static Saeumnis berechne(long forderungCent, LocalDate faelligkeit, List<Rueckzahlung> rueckzahlungen, LocalDate heute) {
        ...
        long bemessungAbgerundet = Math.floorDiv(bemessung, BEMESSUNGSSCHRITT_CENT) * BEMESSUNGSSCHRITT_CENT;
        ...
        if (!stichtag.isAfter(faelligkeit) || bemessung == 0) {
            return new Saeumnis(0, bemessungAbgerundet, 0, rueckgezahlt, rueckstaendig, stichtag, false);
        }
        ...
        if (schonfrist) {
            return new Saeumnis(0, bemessungAbgerundet, 0, rueckgezahlt, rueckstaendig, stichtag, true);
        }
        int monate = 0;
        for (LocalDate grenze = faelligkeit; grenze.isBefore(stichtag); grenze = grenze.plusMonths(1)) {
            monate++;
        }
        long zuschlag = monate * bemessungAbgerundet / 100;
        return new Saeumnis(monate, bemessungAbgerundet, zuschlag, rueckgezahlt, rueckstaendig, stichtag, false);
    }
}
